...
class LetterboxResize(BaseOperator):
    def __init__(self, target_size, pad_val=114., interp=cv2.INTER_LINEAR):
        """
        Resize image preserving aspect ratio (long side fit to target_size),
        then pad the short side so the final canvas is exactly target_size.
        This mirrors YOLO's LetterBox preprocessing and avoids the aspect
        ratio distortion that Resize(..., keep_ratio=False) introduces,
        which is important for small object detection where stretched
        geometry hurts localization.

        Args:
            target_size (int|list): image target size (h, w)
            pad_val (float|list): pixel value used to fill the padded area.
                Defaults to 114 (YOLO-style mid-gray) rather than 0, since 0
                can be mistaken by the network for a valid dark region.
            interp (int): the interpolation method
        """
    ...
    def apply_image(self, image, scale, pad, target_h, target_w):
        im_scale_x, im_scale_y = scale
        top, left = pad
        ...
        if resized.ndim == 3:
            canvas = np.full((target_h, target_w, resized.shape[2]),
                              self.pad_val, dtype=resized.dtype)
        else:
            canvas = np.full((target_h, target_w), self.pad_val,
                              dtype=resized.dtype)
        canvas[top:top + resize_h, left:left + resize_w] = resized
        return canvas

    def apply_bbox(self, bbox, scale, pad, target_size):
        im_scale_x, im_scale_y = scale
        top, left = pad
    ...
    def apply_joints(self, joints, scale, pad, target_size):
        im_scale_x, im_scale_y = scale
        top, left = pad
        resize_w, resize_h = target_size
        joints[..., 0] = joints[..., 0] * im_scale_x + left
        joints[..., 1] = joints[..., 1] * im_scale_y + top
        joints[..., 0] = np.clip(joints[..., 0], 0, resize_w)
        joints[..., 1] = np.clip(joints[..., 1], 0, resize_h)
        return joints

    def apply_segm(self, segms, im_size, scale, pad, target_h, target_w):
        def _resize_poly(poly, im_scale_x, im_scale_y, left, top):
            resized_poly = np.array(poly).astype('float32')
            resized_poly[0::2] = resized_poly[0::2] * im_scale_x + left
            resized_poly[1::2] = resized_poly[1::2] * im_scale_y + top
            return resized_poly.tolist()

        def _resize_rle(rle, im_h, im_w, im_scale_x, im_scale_y, left, top):
            import pycocotools.mask as mask_util
            if 'counts' in rle and type(rle['counts']) == list:
                rle = mask_util.frPyObjects(rle, im_h, im_w)
            ...
            mask = cv2.resize(
                ...
                fy=im_scale_y,
                interpolation=cv2.INTER_NEAREST)
            resize_h, resize_w = mask.shape[:2]
            canvas = np.zeros((target_h, target_w), dtype=mask.dtype)
            canvas[top:top + resize_h, left:left + resize_w] = mask
            rle = mask_util.encode(np.array(canvas, order='F', dtype=np.uint8))
            return rle
        ...
        for segm in segms:
            if is_poly(segm):
                # Polygon format
                resized_segms.append([
                    _resize_poly(poly, im_scale_x, im_scale_y, left, top)
                    for poly in segm
                ])
            else:
                # RLE format
    ...
    def apply(self, sample, context=None):
        """ Resize the image with aspect ratio preserved, then letterbox
        pad to self.target_size.
        """
        im = sample['image']
        if not isinstance(im, np.ndarray):
            raise TypeError("{}: image type is not numpy.".format(self))
        ...
        # single uniform scale so aspect ratio is preserved
        im_scale = min(target_h / im_shape[0], target_w / im_shape[1])
        im_scale_x = im_scale
        im_scale_y = im_scale

        resize_h = int(round(im_shape[0] * im_scale))
        resize_w = int(round(im_shape[1] * im_scale))
        ...
        if 'kps2d' in sample.keys():
            kps2d = sample['kps2d']
            kps2d[:, :, 0] = kps2d[:, :, 0] * im_scale_x + left
            kps2d[:, :, 1] = kps2d[:, :, 1] * im_scale_y + top

            sample['kps2d'] = kps2d

        sample['im_shape'] = np.asarray([target_h, target_w], dtype=np.float32)
        # note: unlike Resize, scale_factor alone can't invert this transform
        # since padding offset also needs to be subtracted; 'pad' is stored
        # separately for that purpose (see un-letterbox note below).
        if 'scale_factor' in sample:
            scale_factor = sample['scale_factor']
            sample['scale_factor'] = np.asarray(
                [scale_factor[0] * im_scale_y, scale_factor[1] * im_scale_x],
            ...
            sample['scale_factor'] = np.asarray(
                [im_scale_y, im_scale_x], dtype=np.float32)
        sample['pad'] = np.asarray([top, left], dtype=np.float32)

        # apply bbox
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            sample['gt_bbox'] = self.apply_bbox(
                sample['gt_bbox'], [im_scale_x, im_scale_y], [top, left],
                [target_w, target_h])
        ...
        if 'semantic' in sample and sample['semantic']:
            semantic = sample['semantic']
            semantic = self.apply_image(
                semantic.astype('float32'), [im_scale_x, im_scale_y],
                [top, left], target_h, target_w)
            semantic = np.asarray(semantic).astype('int32')
            semantic = np.expand_dims(semantic, 0)
            sample['semantic'] = semantic
        ...
        if 'gt_segm' in sample and len(sample['gt_segm']) > 0:
            masks = [
                self.apply_image(gt_segm, [im_scale_x, im_scale_y],
                                 [top, left], target_h, target_w)
                for gt_segm in sample['gt_segm']
            ]
            sample['gt_segm'] = np.asarray(masks, dtype=sample['gt_segm'].dtype)
        ...
        if 'gt_joints' in sample:
            sample['gt_joints'] = self.apply_joints(
                sample['gt_joints'], [im_scale_x, im_scale_y], [top, left],
                [target_w, target_h])
